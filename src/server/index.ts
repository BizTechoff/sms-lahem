...
async function startup() {
    ...
    app.use(sslRedirect());
    app.use(session({
        secret: process.env["NODE_ENV"] === "production" ? process.env['SESSION_SECRET'] : process.env['JWT_SECRET']
        //secret: process.env["NODE_ENV"] === "production" ? process.env['TOKEN_SIGN_KEY'] : process.env['JWT_SECRET']
    }));
    app.use(compression());
    ...
    app.use(api);
    app.use(express.static('dist/sms-lahem'));
    app.use('/*', async (req, res) => {
        ...
        try {
            res.sendFile(process.cwd() + '/dist/sms-lahem/index.html');
        } catch (err) {
            res.sendStatus(500);
        }
    });
